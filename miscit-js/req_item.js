...
//-----------------------------------------------------APPEND ROW------------------------------------------------------
document.getElementById('req_btn').onclick = function() {
    req_row()
}
...
function req_row() {
    ...
    $('#item_req').append('<div class="row item_row" id="req_row' + x + '" >' +
        //item_type
        '<div class="col_type"><select class="form form-control z-depth-1 item_type er_span" onchange="show_info_0(id)" id="item_type' + x + '">' +
        ...
        //item_name
        '<div class="col col_name"><select class="form-control z-depth-1 item_name" onchange="show_info_1(id)" id="item_name' + x + '" disabled>' +
        '<option value="0"></option>' +
        '</select>' +
        '<span class="error_data" id="error_name' + x + '"></span></div>' +
        //item_unit
        '<div class="col_unit"><input class="form-control z-depth-1 item_unit" type="text" id="item_unit' + x + '" oninput="check_val(id)" max="0" style="z-index: -1;" disabled></input>' +
    ...
    if (x == 'product') {
        $.ajax({
            ...
            success: function(data) {
                ...
                //---------------- remove โปรดระบุ -----------------
                let err_id = document.getElementById(id).nextElementSibling.id;
                $('#' + err_id).html('');
            }
        })
    };
...
function show_info_1(id) {
    ...
    for (let k = 0; k <= h1; k = k + 2) { //loop for check every row
        ...
        // console.log(dup_itempd_name)
        if (c_k_type == 'product') {
            if (c_k_name == this_value) {
                dup_itempd_name++;
            }
        }
        if (c_k_type == 'product_id') {
            if (c_k_name == this_value) {
                dup_itempd_name++;
            }
        }
...
document.getElementById('send_data').onclick = async function() {
    ...
    await axios.post('./request_item/item_v', check_data)
        .then((data) => {
        ...
        success: function(data) {
            ...
            if (data.firstname != null && sum == 0 && req_data != 0) {
                ...
                let confirm = document.getElementById('confirm').onclick = async function() {
                    ...
                    /* --------------------------- Send Email ---------------------------- */
                    await $.ajax({
                        url: "./request_item/send_email_data",
                        method: "post",
                        dataType: "json",
                        data: {
                            'firstname': firstname,
...
document.getElementById('dis_modal').onclick = () => {
    $('#form_ready').modal('hide');
}
