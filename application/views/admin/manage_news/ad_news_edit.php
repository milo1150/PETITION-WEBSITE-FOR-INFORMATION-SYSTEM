...
<div class="wrapper">
	...
	<body>
		...
		<div class="table-responsive">
			<p class="text-center text-dark" style="margin-top: 15px; font-size: 35px; font-weight: bold;">แก้ไขรายการข่าว</p>
				...
				<div class="table-responsive-md pb-4 pt-4">
					...
					<div class="container" style="max-width:1000px;">
						<div class="form-group">
							<label style="padding-left: 4px;">ประเภท</label>							
								<select class="form-control" name="ntype" style="border-color: black;">
		            				<option value="ข่าวทั่วไป" <?php if($type == 'ข่าวทั่วไป'){echo 'selected';}?>>ข่าวทั่วไป</option>
									<option value="ข่าวมหาวิทยาลัย" <?php if($type == 'ข่าวมหาวิทยาลัย'){echo 'selected';}?>>ข่าวมหาวิทยาลัย</option>
									<option value="ข่าวสารสนเทศ" <?php if($type == 'ข่าวสารสนเทศ'){echo 'selected';}?>>ข่าวสารสนเทศ</option>
									<option value="อื่นๆ">อื่นๆ</option>
		            			</select>
						</div>
						<div class="form-group">
							<label>หัวข้อ</label>
							<input type="text" name="title" id="title_v" class="form-control" style="border-color: black;" value="<?php echo $title?>">
						...
						<div class="form-group">
							<label>รูป</label>						
							<div class="form-group" >						
								<img id="blah" src="<?php echo base_url().'uploads/'.$img_name?>" />
							</div>
						</div>
						...
						<div class="form-group">
							<label>เนื้อหา</label>
							<div style="display:none" id="v_content"><?php echo $content;?></div>
...
$(document).ready(function(){
	setTimeout(function(){
		...
		let txt_inner = document.getElementById('txtEditor').parentNode.childNodes[6].childNodes[0].childNodes[1].childNodes[0].childNodes[0].contentWindow.document.childNodes[1].childNodes[1]
		//txt_inner.innerHTML = 'dddd'
		//let s = txt_inner.innerHTML
		//console.log(txt_inner.innerHTML)

		let content_data = document.getElementById('v_content').innerHTML
		txt_inner.innerHTML = content_data
...
$('#form-test').on('submit',function(e){
	...
	const txt_inner = document.getElementById('txtEditor').parentNode.childNodes[6].childNodes[0].childNodes[1].childNodes[0].childNodes[0].contentWindow.document.childNodes[1].childNodes[1].innerHTML
	//console.log(img_n)
	//console.log(title_v)
	...
	$('#send_inf').click(function(){
		...
		let id = '<?php echo $id;?>';
			$.ajax({
				url:"<?php echo base_url()?>Ad_news/news_edit_conf",
				method: 'POST',
				data: x1,
				dataType: 'json',
				contentType: false,
