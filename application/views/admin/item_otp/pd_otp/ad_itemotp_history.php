				...
				<table class="table table-bordered table-hover mytable text-nowrap shadow" style="margin-top:10px;  ">
					<thead class="thead-light z-depth-1">
						...
						<tbody>
							...
							?>						
								<tr>
									<td class="text-center font-weight-bolder <?php if($row->order_status == 0){echo "red accent-3";}if($row->order_status == 2){echo "green accent-3";}if($row->order_status == 3){echo "purple lighten-1";}?>" style="padding-top:21px; font-size:16px;" ><?php echo $row->id;?></td>
									<td class="text-center font-weight-bolder" style="padding-top:21px; font-size:16px;"><?php if($row->order_status==0){echo 'ยังไม่ได้ดำเนินการ';} if($row->order_status==3){echo 'ยกเลิกรายการ';} if($row->order_status==2){echo 'เสร็จสมบูรณ์';}?></td>
									...
									<form action="<?php if($row->order_status == 0){echo base_url().'ad_itemotp/report_itemotp_order';} if($row->order_status == 2){echo base_url().'ad_itemotp/report_itemotp_com';}if($row->order_status == 3){echo base_url().'ad_itemotp/report_itemotp_cancle';}?>" method="POST"><button value="<?php echo $row->id;?>" name="id"  type="submit" class="btn cyan lighten-3"><i class="fas fa-search" ></i></button></form>																		
									</td>
