...
class ad_fix extends CI_Controller {
	...
	public function ad_fix_alldata(){
		$this->load->view('ad_fix_alldata');
	}
	public function filter_serch(){
		$data = array();
		if($_GET['fixlist']!=null){
			$fixlist = $this->input->get('fixlist');
			if($fixlist == "internet"){
				$data[0] = "แจ้งปัญหาระบบอินเทอร์เน็ต";
			}
			if($fixlist == "website"){
				$data[0] = "แจ้งปัญหาเว็บไซต์";
			}
			if($fixlist == "network"){
				$data[0] = "แจ้งปัญหาระบบเครือข่าย";
			}
			if($fixlist == "internet"){
				$data[0] = "แจ้งปัญหาระบบอินเทอร์เน็ต";
			}
			if($fixlist == "printer"){
				$data[0] = "แจ้งปัญหาปริ้นเตอร์";
			}
			if($fixlist == "item"){
				$data[0] = "แจ้งอุปกรณ์ชำรุด";
			}
			if($fixlist == "computer"){
				$data[0] = "แจ้งปัญหาคอมพิวเตอร์";
			}
		}
		...
		$data = array();
		if($_GET['fixlist']!=null){
			$data[0] = $this->input->post('fixlist');
		}
		if($_GET['building']!=null){
			$data[1] = $this->input->post('building');
		}
		if($_GET['floor']!=null){
			$data[2] = $this->input->post('floor');
		}
		if($_GET['room']!=null){
			$data[3] = $this->input->post('room');
		}
